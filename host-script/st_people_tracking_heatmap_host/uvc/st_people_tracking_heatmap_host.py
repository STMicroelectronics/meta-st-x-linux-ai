...
def decode_process(queue_message, baudrate=9600):
    serial_port_name = ""
    ports = serial.tools.list_ports.comports()
    for port in ports:
        if "STM32MP2" in port.description: # Linux
            print(f"Selected device: {port.device}")
            print(f"Description: {port.description}")
            serial_port_name = port.device
        if "0483:5740" in port.hwid: # Windows
            print(f"Selected device: {port.device}")
            print(f"Description: {port.description}")
            serial_port_name = port.device

    try:
        serial_port = serial.Serial(port=serial_port_name, baudrate=baudrate, timeout=0.5)
        print(f"Serial port {serial_port_name} opened.")
    ...
    try:
        while True:
            ...
            if bytes_available > 0:
                ...
                data_chunk = data_bytes.decode('utf-8', errors='ignore')
                ...
                while True:
                    ...
                    end_idx = data_buffer.find('/E')

                    if start_idx != -1 and end_idx != -1 and start_idx < end_idx:
                        message = data_buffer[start_idx + 2:end_idx].strip()
                        if "B" in message:
                            print("B found in message skip this message")
                        elif "E" in message:
                            print("E found in message skip this message")
                        else:
    ...
    if os_used == "Linux":
        ...
        for dev in os.listdir('/dev'):
            if dev.startswith('video'):
                ...
                try:
                    ...
                    for line in result.stdout.splitlines():
                        if line.strip().startswith('Driver name'):
                            # Just to confirm device is valid
                            continue
                ...
                video_devices.append((path, name))
        ...
        for i in range(max_tested):
            ...
            if cap is None or not cap.isOpened():
                cap.release()
                continue
            # Try to grab a frame to confirm camera works
            ret, frame = cap.read()
            if ret:
...
def print_devices(devices):
    print("\nAvailable Cameras:")
    if os_used == "Linux":
        for idx, (dev, name) in enumerate(devices):
    ...
    print("\nEnter the number to select a camera, 'r' to refresh, or 'q' to quit.")

def camera_process(queue_frame, frame_width, frame_height, video_index):
    ...
    # Open a connection to the video device (e.g., webcam)
    cap = open_camera(video_index.value)
    vidx = video_index.value

    # Check if the video device was opened successfully
    if not cap.isOpened():
    ...
    while True:
        ...
        if ret:
            ...
            # Show a black frame with error text
            import numpy as np
            error_frame = np.zeros((480, 640, 3), dtype=np.uint8)
            cv2.putText(error_frame, f"Camera {video_index.value} not available",
                        (30, 240), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,0,255), 2)
        if vidx != video_index.value:
            print("vidx: ", video_index.value)
            vidx = video_index.value
            cap.release()
            cap = open_camera(video_index.value)
            if cap is None:
                print(f"[ERROR] Camera {video_index.value} not available. Staying on previous camera.")
                # Try to reopen the previous camera
                current = (video_index.value - 1)
...
def show_process(queue_frame, queue_message, frame_width, frame_height, video_index, max_cam):
    def update_frame():
    ...
    def button_camera_click():
        if video_index.value < max_cam-1:
            video_index.value += 1
        else:
            video_index.value = 0
        print(f"Video index: {video_index.value}")
    ...
    tabview.tab("Customization").grid_rowconfigure(4, weight=1)
    tabview.tab("Customization").grid_rowconfigure(5, weight=1)
    tabview.tab("Customization").grid_rowconfigure(6, weight=1)
    ...
    theme_optionemenu.grid(row=1, column=0, padx=10, pady=10, sticky="w")
    ...
    scaling_optionemenu.grid(row=3, column=0, padx=10, pady=10, sticky="w")
    scaling_optionemenu.set("1280x720")
    width_slide, height_slide = 1280, 720
    scaling_optionemenu_label = ctk.CTkLabel(tabview.tab("Customization"), text="Video settings:", anchor="w")
    scaling_optionemenu_label.grid(row=2, column=0, sticky="sw")
    # Change camera/video index
    button_v = ctk.CTkButton(tabview.tab("Customization"), text="Change camera", command=button_camera_click, fg_color="#2e4664")
    button_v.grid(row=4, column=0, padx=10, pady=10, sticky="w")

    # Print text box
    text_button_label = ctk.CTkLabel(tabview.tab("Customization"), text="Disable text:", anchor="w")
    text_button_label.grid(row=5, column=0, sticky="w")
    button_t = ctk.CTkSwitch(tabview.tab("Customization"), text="Print/Remove text", command=print_textbox_content, fg_color="#2e4664")
    button_t.grid(row=6, column=0, padx=10, pady=10, sticky="nsew")
...
if __name__ == "__main__":
    ...
    video_index = mp.Value('i', 0)
    queue_frame = mp.Queue(maxsize=1)
    queue_message = mp.Queue(maxsize=1)

    def signal_handler(sig, frame):
        print("\nDestroying process...")
        for p in [cam_process, msg_process, shw_process]:
            if p.is_alive():
                p.terminate()
                p.join()
    ...
    while True:
        ...
        print("  [0] Windows")
        print("  [1] Linux")
        choice_os = input("Your choice: ").strip()
        if choice_os.isdigit():
            value_os = int(choice_os)
            if value_os == 0:
                print(f"Selected OS Windows (value :{value_os})")
                os_used = "Windows"
            elif value_os == 1:
                print(f"Selected OS Linux (value :{value_os})")
                os_used = "Linux"
            else:
                print(f"Unrecognized value {value_os} for OS")
            break
    ...
    while True:
        ...
        max_cam = len(devices)
        print("Number of camera detected: ", max_cam)
        ...
        if choice.lower() == 'q':
            ...
            video_index.value = int(choice)
            if os_used == "Linux":
                if 0 <= video_index.value < len(devices):
                    print(f"Selected camera: /dev/video{video_index.value}")
                    # Here you can add code to open/use the selected device
                    break
                else:
                    print("Invalid selection. Try again.")
            elif os_used == "Windows":
                if video_index.value in devices:
                    print(f"Selected camera index: {video_index.value}")
                    # You can add code here to open and use the camera
                    break
                else:
                    print("Invalid selection. Try again.")
        else:
    ...
    # Setup & start procs
    cam_process = mp.Process(target=camera_process, args=(queue_frame, frame_width, frame_height, video_index))
    cam_process.start()

    msg_process = mp.Process(target=decode_process, args=(queue_message, 9600))
    msg_process.start()

    shw_process = mp.Process(target=show_process, args=(queue_frame, queue_message, frame_width, frame_height, video_index, max_cam))
    shw_process.start()
    ...
    if os_used == "Linux":
        try:
            while True:
                signal.pause()  # On Windows, signal.pause() is not available; use sleep loop instead
