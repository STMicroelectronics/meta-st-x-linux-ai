...
void GetObjDetectResults(Config* conf, Interpreter* interpreter, ObjDetect_Results* results, std::shared_ptr<edgetpu::EdgeTpuContext>* edgetpu_context)
{
	interpreter->impl->SetExternalContext(kTfLiteEdgeTpuContext, edgetpu_context->get());
	std::vector<std::pair<float, int>> top_results;

	// location
	float *locations = interpreter->impl->typed_tensor<float>(1);
	// classes
	float *classes = interpreter->impl->typed_tensor<float>(2);
	// score
	float *scores = interpreter->impl->typed_tensor<float>(3);
