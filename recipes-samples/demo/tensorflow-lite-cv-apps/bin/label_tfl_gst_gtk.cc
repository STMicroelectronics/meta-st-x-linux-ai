...
struct tflite::wrapper_tfl::TfL_Results results;
...
/**
 * This function is an helper to get the current time in milliseconds
 */
...
static void nn_inference(uint8_t *img)
{
	...
	inference_time = (get_ms(stop_time) - get_ms(start_time));

	tflite::wrapper_tfl::TfLiteGetResults(&config, interpreter, &results);
...
static void gui_create(CustomData *data)
{
	...
	if (data->preview_enabled)
		data->brain_icon = cairo_image_surface_create_from_png("/usr/local/demo-ai/ai-cv/resources/ST7079_AI_neural_pink_65x55.png");
	else
		data->brain_icon = cairo_image_surface_create_from_png("/usr/local/demo-ai/ai-cv/resources/ST7079_AI_neural_pink_65x80_next_inference.png");
	data->exit_icon = cairo_image_surface_create_from_png("/usr/local/demo-ai/ai-cv/resources/close_50x50_pink.png");
